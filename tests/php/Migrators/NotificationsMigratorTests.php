...
namespace Automattic\WooCommerce\StockNotificationsMigrator\Tests\Migrators;
...
class NotificationsMigratorTests extends WC_Unit_Test_Case {
	...
	public function test_ineligible_rows_are_skipped_with_a_recorded_outcome(): void {
		...
		$outcomes = $this->migrator->migrate_batch( $batch, Container::get( Writer::class ) );
	...
	public function test_an_already_migrated_row_is_skipped_without_an_outcome(): void {
		...
		$outcomes = $this->migrator->migrate_batch( array( $legacy_id ), Container::get( Writer::class ) );
	...
	public function test_dry_run_writes_nothing_and_reports_the_same_shape(): void {
		...
		$real_outcomes = $real_migrator->migrate_batch( $ids, Container::get( Writer::class ) );
	...
	public function test_admitted_rows_equal_migrated_plus_adopted_plus_failed(): void {
		...
		$outcomes = $this->migrator->migrate_batch( $admitted, Container::get( Writer::class ) );
	...
	public function test_legacy_meta_is_read_once_per_batch(): void {
		...
		$this->migrator->migrate_batch( $batch, Container::get( Writer::class ) );
	...
	public function test_a_pending_row_carries_the_verification_digest_and_not_the_secrets(): void {
		...
		$this->assertTrue( LegacyHash::verify_token( $meta_value, (string) $token ) );
	...
	private function migrate_all(): array {
		return $this->migrator->migrate_batch( $this->migrator->get_batch( 0, 500 ), Container::get( Writer::class ) );
	}
