...
use Automattic\WooCommerce\StockNotificationsMigrator\Migrators\ProductMetaMigrator;
...
class ProductMetaMigratorTests extends WC_Unit_Test_Case {
	...
	public function test_a_variation_is_settled_without_being_written(): void {
		...
		$migrator = $this->build_migrator();
		$outcomes = $migrator->migrate_batch( $migrator->get_batch( 0, 10 ), Container::get( Writer::class ) );
	...
	public function test_a_throwing_save_hook_settles_the_row(): void {
		...
		try {
			$outcomes = $migrator->migrate_batch( $batch, Container::get( Writer::class ) );
		} finally {
