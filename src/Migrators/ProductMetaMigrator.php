...
namespace Automattic\WooCommerce\StockNotificationsMigrator\Migrators;
...
class ProductMetaMigrator implements MigratorInterface {
	...
	public function count_remaining( int $cursor = 0 ): int { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- part of MigratorInterface; see above.
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql was prepared by candidate_sql(); table names are fixed internal names, never user input. The candidate predicate is a NOT EXISTS over postmeta that no WP_Query expresses, and the count is the section's remaining work, which every migrated batch changes, so a cached value would report work as done that the store has not had.
