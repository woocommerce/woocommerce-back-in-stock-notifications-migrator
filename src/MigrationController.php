...
class MigrationController {
	...
	public function register(): void {
		// The `wp wc bis-migrate` command registers itself from the plugin bootstrap, on
		// `after_wp_load`, like every other WooCommerce command. Registering here would build
		// the command object on every `wp` invocation, `wp help` included.
		...
		add_filter( 'woocommerce_get_batch_processor', array( $this, 'provide_batch_processor' ), 10, 2 );
	}

	/**
	 * Hand WooCommerce's batch processing controller a wired-up migration processor.
	 *
	 * The controller resolves processors from Core's container, which only knows Core's own
	 * classes, and its fallback would construct this one without the dependencies `init()`
	 * supplies. So the instance comes from this plugin's own container instead.
	 *
	 * @param object|null $processor            Instance Core resolved, if any.
	 * @param string      $processor_class_name Class name Core is resolving.
	 * @return object|null
	 */
	public function provide_batch_processor( $processor, string $processor_class_name ) {
		if ( MigrationBatchProcessor::class !== $processor_class_name ) {
			return $processor;
		}

		return Container::get( MigrationBatchProcessor::class );
	}
	...
	public function handle_woocommerce_debug_tools( array $tools ): array {
		return Container::get( ToolsRegistrar::class )->handle_woocommerce_debug_tools( $tools );
	}
