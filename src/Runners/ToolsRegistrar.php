...
namespace Automattic\WooCommerce\StockNotificationsMigrator\Runners;

use Automattic\WooCommerce\Internal\BatchProcessing\BatchProcessingController;
use Automattic\WooCommerce\StockNotificationsMigrator\Container;
use Automattic\WooCommerce\StockNotificationsMigrator\MigrationRun;
use Automattic\WooCommerce\StockNotificationsMigrator\MigrationState;
use Automattic\WooCommerce\StockNotificationsMigrator\Report\Reporter;
use Automattic\WooCommerce\StockNotificationsMigrator\Requirements;
...
class ToolsRegistrar {
	...
	public function handle_woocommerce_debug_tools( array $tools ): array {
		...
		if ( $is_running ) {
			$tools['stop_bis_migration'] = array(
				'name'     => __( 'Stop migrating Back In Stock Notifications subscribers', 'back-in-stock-notifications-migrator-for-woocommerce' ),
				'button'   => __( 'Stop migration', 'back-in-stock-notifications-migrator-for-woocommerce' ),
				'desc'     => $description,
		...
		$tools['start_bis_migration'] = array(
			'name'     => __( 'Migrate Back In Stock Notifications subscribers', 'back-in-stock-notifications-migrator-for-woocommerce' ),
			'button'   => __( 'Start migration', 'back-in-stock-notifications-migrator-for-woocommerce' ),
			'desc'     => $description,
	...
	public function start(): string {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			return __( 'You do not have permission to do this.', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}
		...
		if ( $batch_processor->is_enqueued( MigrationBatchProcessor::class ) ) {
			return __( 'Migration already in progress, nothing done.', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}
		...
		if ( ! $migration_state->acquire_lock( self::BACKGROUND_LOCK_OWNER ) ) {
			...
			return sprintf(
				...
				__( 'A migration is already running (%s). Stop it there first; starting one here does not override it.', 'back-in-stock-notifications-migrator-for-woocommerce' ),
				$lock['owner'] ?? __( 'unknown', 'back-in-stock-notifications-migrator-for-woocommerce' )
			);
		}
		...
		return __( 'Migration started. Subscribers will be migrated in the background over the next few minutes.', 'back-in-stock-notifications-migrator-for-woocommerce' );
	}
	...
	public function stop(): string {
		...
		if ( ! $batch_processor->is_enqueued( MigrationBatchProcessor::class ) ) {
			return __( 'Migration not in progress, nothing done.', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}
		...
		return __( 'Migration stopped. Subscribers already moved stay put, and the next run picks up where this one left off.', 'back-in-stock-notifications-migrator-for-woocommerce' );
	}
	...
	private function get_description( bool $is_running ): string {
		$lines = array( __( 'Moves subscribers and settings from Back In Stock Notifications into the built-in stock notifications. Runs in the background, a batch at a time.', 'back-in-stock-notifications-migrator-for-woocommerce' ) );
	...
	private function get_failure_line( bool $is_running ): string {
		...
		$failure = Container::get( MigrationState::class )->get_failure();
	...
	private function get_parked_line(): string {
		$parked = Container::get( MigrationState::class )->get_parked_sections();
	...
	private function get_progress_line( bool $is_running ): string {
		...
		if ( null === $cached ) {
			return __( 'Not started yet. Start the migration to see how much there is to move.', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}
		...
		if ( 0 === $remaining ) {
			$headline = sprintf(
				...
				__( 'Every subscriber has been checked, as of %s.', 'back-in-stock-notifications-migrator-for-woocommerce' ),
				$as_of
			);
		} elseif ( $is_running ) {
			$headline = sprintf(
	...
	private function get_progress( MigrationState $migration_state, int $remaining ): string {
		...
		return sprintf(
			...
			__( '%1$s of %2$s checked (%3$d%%).', 'back-in-stock-notifications-migrator-for-woocommerce' ),
	...
	private function get_settings_sections_line( MigrationState $migration_state ): string {
		...
		if ( null === $cached || (int) $cached['count'] > 0 ) {
			$outstanding[] = __( 'product settings', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}

		if ( ! ( new MigrationRun() )->get_options_migrator()->is_done() ) {
			$outstanding[] = __( 'store settings', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}

		if ( empty( $outstanding ) ) {
			return __( 'Product settings and store settings have been imported.', 'back-in-stock-notifications-migrator-for-woocommerce' );
		}

		return sprintf(
			/* translators: %s: comma-separated list of the settings still to import, e.g. "product settings, store settings" */
			__( 'Still to import: %s.', 'back-in-stock-notifications-migrator-for-woocommerce' ),
			implode( ', ', $outstanding )
		);
	}
	...
	private function get_losses_line(): string {
		$cached_losses = Container::get( MigrationState::class )->get_losses();
		...
		return sprintf(
			...
			__( 'Skipped so far, as of %s:', 'back-in-stock-notifications-migrator-for-woocommerce' ),
